...
def download_pdfs_from_csv(csv_path, start_line=1):
    ...
    for header in reader:
        ...
        print("Header row with 'Category' and 'Name of medicine' not found.")
        csvfile.close()
        return
    ...
    for row in reader:
        row_count += 1
        if len(row) <= idx_pdf:
            print(f"Line {row_count}: Not enough columns.")
            continue
        ...
        try:
            ...
            if resp.status_code == 429:  # Too Many Requests
                print(f"---- Error 429 (Too Many Requests) at line {row_count}. Waiting 5 minutes before retrying.")
            ...
            if resp.status_code == 200 and resp.content.startswith(b'%PDF'):
                ...
                if alt_resp.status_code == 200 and alt_resp.content.startswith(b'%PDF'):
                    ...
                    if withdrawal_resp.status_code == 200 and withdrawal_resp.content.startswith(b'%PDF'):
                        ...
                        print(f"Error when downloading {med_name_url}")
        except Exception as e:
            print(f"Error with {pdf_url} or alternative URL: {e}")
    csvfile.close()
    print(f"Processed lines: {row_count}")
    print(f"New downloads: {download_count}")

def main():
    csv_path = r"./DrugFork/data/EMA/EMA_CSV_UTF8_3.csv" # TODO change path to the correct file within the repo

...
# delete rows from START_ROW_TO_CLEAR to the end of the sheet
...
# columns in Google Sheet (must be in correct order for upload)
columns_gsheet = [
...
# prepare empty DataFrame with correct columns
df_sheet = pd.DataFrame(columns=columns_gsheet)

# enter data into DataFrame with correct mapping and formatting
...
# leave remaining columns empty for now
for col in columns_gsheet:
    if col not in df_sheet.columns:
        df_sheet[col] = ""

# columns in correct order for upload
df_sheet = df_sheet[columns_gsheet]

# insert new data from START_ROW_TO_CLEAR downwards
